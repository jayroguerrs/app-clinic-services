...
{
	public class FacturaSerieDat : IFacturaSerieDat
	{
        private readonly string _connectionString;
        public FacturaSerieDat(string connectionString)
        {
        ...
        public async Task<List<FacturaSerieDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("SP_FacturaSerie_Listar", conn)
                {
        ...
        public async Task<List<FacturaSerieDTO>> ListarByEstado(int idEstado)
        {
            try
            {
                using SqlConnection conn = GetConnection();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("SP_FacturaSerie_ListarByEstado", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Registrar(FacturaSerieDTO model)
        {
            try
            {
                using SqlConnection conn = GetConnection();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("SP_FacturaSerie_Registrar", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Modificar(int id, FacturaSerieDTO model)
        {
            try
            {
                using SqlConnection conn = GetConnection();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("SP_FacturaSerie_Modificar", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        static async Task<List<FacturaSerieDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<FacturaSerieDTO> collection = new List<FacturaSerieDTO>();
                while (await reader.ReadAsync())
                {
                    FacturaSerieDTO obj = new FacturaSerieDTO();
