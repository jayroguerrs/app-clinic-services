using System;
using System.Collections.Generic;
using System.Text;

namespace DepilZone.Entidad
{
	public class EmpresaEnt
	{
		public int IdEmpresa { get; set; }
		public string NombreComercialEmisor { get; set; }
		public string RazonSocialEmisor { get; set; }
		public int UbigeoEmisor { get; set; }
		public string DireccionEmisor { get; set; }
		public string TextoEmisor { get; set; }
		public string DireccionLocal { get; set; }
		public string TextoLocal { get; set; }
		public string TextoLocal2 { get; set; }
		public string TextoLocal3 { get; set; }
		public string UrbanizacionEmisor { get; set; }
		public string ProvinciaEmisor { get; set; }
		public string DepartamentoEmisor { get; set; }
		public string DistritoEmisor { get; set; }
		public string CodigoPaisEmisor { get; set; }
		public string CorreoEmisor { get; set; }
		public string RucEmisor { get; set; }

		public string TelefonosEmisor { get; set; }
	}
}
